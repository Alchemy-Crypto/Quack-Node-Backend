...
module.exports = class AdminService {
    ...
    static async initialDistribution() {
        ...
        // });
        const verificationArray = await Promise.all(members.map((member) => {
            console.log('line 50', member)
            return contract.methods.distributeQuacks(member, quackPerMember).send({from: custodialAddress}).on('receipt', (receipt) => { return receipt });
        }));
